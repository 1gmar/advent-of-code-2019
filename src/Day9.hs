...
outputTestResult state@ProgramState {..} instr@Instruction {..} =
  case instr of
    Instruction _ (param:_) -> withParam param >>= andOpCode >>= logTest
    _                       -> illegalProgramState iPointer program
  where
    nextP = nextIPointer operation iPointer
    withParam param = paramOf param state
    andOpCode (value, prog) = (value, ) <$> prog `elemAt` iPointer
    logTest (value, (opCode, prog)) = runIntCodeProgram $ nextState value opCode prog
    nextState value opCode prog = state {iPointer = nextP, result = result ++ [output value opCode], program = prog}
    output value opCode = concat ["Test result for opcode: ", opCode, " at ", show iPointer, ": ", show value]
